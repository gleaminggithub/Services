...
const RatingModal: FC<Props> = (props) => {
  ...
  return (
    ...
    >
      <div className='bg-white w-96 p-4 rounded-lg shadow-lg'>
        ...
        <div className='mb-4'>
            ...
            {starValues.map(value=><button className={`w-6 h-6 ${ratingValue === value ? 'text-yellow-500':'text-gray-300'}`} onClick={()=>setRatingValue(value)} key={value}>
                <BsStarFill/>
            </button>)}
            </div>
        </div>
    ...
    <div className='flex justify-end'>
        <button onClick={reviewSubmitHandler}  disabled={isSubmittingReview} className='px-4 bg-primary text-white rounded-md'>{isSubmittingReview ? "Submitting" : "Submit"}</button>
        <button
